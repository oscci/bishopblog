...
If there is a big influence of random error, then the test score is likely to change from one occasion to the next. Suppose we measure the same children on two occasions a month apart on our three tests, and then plot scores on time 1 vs time 2. To simplify this example, we assume that all three tests have the same normal distribution of scores (as for test A in Figure 1).


```{r corrplots }
#We'll now add random error to A, B and C , at different levels to reflect different reliability
myrands <- rnorm((myN*3),0,30)
